...
// ── Abuse Appeal Modal Submit ─────────────────────────────────────────────
if (interaction.customId && interaction.customId.startsWith('modal_abuse_appeal_')) {
  await interaction.deferReply({ ephemeral: true }).catch(() => {});
  try {
    ...
    const recordId = interaction.customId.replace('modal_abuse_appeal_', '');
    const appealMessage = interaction.fields.getTextInputValue('appeal_message');

    // Dileğe kaydını güncelle
    const record = abuseLog.appealAbuse(recordId, appealMessage);
    if (!record) {
    ...
    const embed = new EmbedBuilder()
      .setColor(0x3498DB)
      .setTitle('📨 Dileğe Başarıyla Gönderildi')
      .setDescription(
        `✅ Abuse dileğeniz başarıyla sisteme kaydedilmiştir.\n\n` +
        `**Dileğe Bilgileri:**\n` +
        `• Kayıt ID: \`${record.id}\`\n` +
        `• İşlem Türü: ${record.abuseType}\n` +
        `• Durumu: ⏳ Beklemeyde\n\n` +
        `👮 Moderatörlük ekibi tarafından incelenecek ve sonuç DM olarak iletilecektir.`
      )
      .setFooter({ text: 'Eko Yıldız • Abuse Sistemi' })
      .setTimestamp();

    // Mod kanalına bildirim gönder
    const guild = await interaction.client.guilds.fetch(GUILD2_ID).catch(() => null);
    if (guild) {
      const modChan = guild.channels.cache.find(c => c.name === 'mod-durum-raporları' || c.name === 'moderator-reports');
      if (modChan) {
        const noticeEmbed = abuseLog.createAppealNoticeEmbed(record);
        const { ActionRowBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
        const row = new ActionRowBuilder().addComponents(
          new ButtonBuilder()
            .setCustomId(`approve_appeal_${record.id}`)
            .setLabel('✅ Onayla (Ceza Kaldır)')
            .setStyle(ButtonStyle.Success),
          new ButtonBuilder()
            .setCustomId(`reject_appeal_${record.id}`)
            .setLabel('❌ Reddet')
            .setStyle(ButtonStyle.Danger)
        );

        await modChan.send({ embeds: [noticeEmbed], components: [row] }).catch(() => {});
      }
    }

    return interaction.editReply({ embeds: [embed] });
  } catch (err) {
    console.error('[abuse_appeal_modal] Error:', err.message);
    return interaction.editReply({ content: `❌ Hata: ${err.message}` });
  }
}
